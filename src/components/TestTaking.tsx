            ...
            <div className="mb-6">
              ...
              <div className="space-y-3">
                {test.questions[currentQuestion].options.map((option, index) => (
                  <label
                    key={index}
                    className="flex items-start p-3 sm:p-4 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors"
                  >
                    <input
                      type="radio"
              ...
              {test.questions[currentQuestion].type === 'true-false' && (
                <div className="space-y-3">
                  <label className="flex items-center p-4 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                    ...
                    />
                    <span className="ml-3 text-gray-900 text-sm sm:text-base">True</span>
                  </label>
                  <label className="flex items-center p-4 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                    <input
                      type="radio"
                      ...
                      className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300"
                    />
                    <span className="ml-3 text-gray-900 text-sm sm:text-base">False</span>
                  </label>
                </div>
              )}
              ...
              {test.questions[currentQuestion].type === 'fill-in-blank' && (
                <div>
                  <p className="text-gray-600 mb-3 text-sm sm:text-base">Fill in the blank:</p>
                  <input
                    type="text"
                    value={answers[test.questions[currentQuestion].id] as string || ''}
                    onChange={(e) => handleAnswerSelect(test.questions[currentQuestion].id, e.target.value)}
              ...
              {test.questions[currentQuestion].type === 'real-number' && (
                <div>
                  <p className="text-gray-600 mb-3 text-sm sm:text-base">Enter the numerical answer (up to 3 decimal places):</p>
                  <input
                    type="number"
                    step="0.001"
                    value={answers[test.questions[currentQuestion].id] as string || ''}
